...
DescriptorHeapAllocation CPUDescriptorHeap::Allocate(uint32_t count) {
    ...
    if (allocation.IsNull()) {
        ...
        m_HeapDesc.NumDescriptors = 
            (m_HeapDesc.NumDescriptors > static_cast<UINT>(count)) ? m_HeapDesc.NumDescriptors : static_cast<UINT>(count);
    ...
    m_MaxSize = (m_MaxSize > m_CurrentSize) ? m_MaxSize : m_CurrentSize;
