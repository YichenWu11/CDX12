...
bool DX12App::InitDirect3D()
{
	...
	// Enable the D3D12 debug layer. (启用D3D12的调试层)
	{
	...
	CreateCommandObjects();             // 创建命令队列（GPU），命令列表和命令分配器（CPU）
	CreateSwapChain();					// 创建交换链
	CreateRtvAndDsvDescriptorHeaps();   // 创建 Rtv 和 Dsv 描述符堆
...
void DX12App::CreateCommandObjects()
{
	...
	ThrowIfFailed(mDevice->CreateCommandAllocator(
		D3D12_COMMAND_LIST_TYPE_DIRECT,  // direct ---> 存储一系列可供 GPU 直接执行的命令
	...
	ThrowIfFailed(mDevice->CreateCommandList(
		...
		nullptr,                   // Initial PipelineStateObject （初始化流水线状态对象）无绘制对象
